package org.openmrs.module.ohrireports.reports.datasetvaluator.datim.tx_tb_numerator;
...
public class TxTbNumeratorAutoCalculateDataSetDefinitionEvaluator implements DataSetEvaluator {
	...
	public int getTBstartedInReportingPeriod() {
		...
		queryBuilder.select("obs")
				.from(Obs.class, "obs")
				.whereEqual("obs.concept", conceptService.getConceptByUuid(TB_TREATMENT_START_DATE))
				.and()
	...
	public List<Integer> getDispenseDose() {
		...
		queryBuilder.select("obs").from(Obs.class, "obs")
				...
				.whereIdIn("obs.personId", pList).whereLess("obs.obsDatetime", hdsd.getEndDate())
				.orderDesc("obs.personId,obs.obsDatetime");
