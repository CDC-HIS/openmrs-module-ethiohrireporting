...
public class HMISReport implements ReportManager {
	...
	public ReportDefinition constructReportDefinition() {
		...
		// aDefinition.setDescription("06 - HIV | Hospital, Health center, Clinic |
		// Monthly (Federal Ministry Of Health)");
		// reportDefinition.addDataSetDefinition(
		// "HMIS:06 - HIV | Hospital, Health center, Clinic | Monthly (Federal Ministry
		// Of Health)",
		...
		// reportDefinition.addDataSetDefinition("HMIS:Number of adults and children
		// with HIV infection newly started on ART",
		// map(txNewDataset, "startDate=${startDateGC},endDate=${endDateGC}"));

		// HIVARTRETDatasetDefinition hivArtRetDatasetDefinition = new
		// HIVARTRETDatasetDefinition();
		// hivArtRetDatasetDefinition.addParameters(getParameters());
		// hivArtRetDatasetDefinition.setNetRetention(false);
		// reportDefinition.addDataSetDefinition(
		// "HMIS:Number of adults and children who are still on treatment at 12 months
		// after\n" + //
		// "initiating ART", map(hivArtRetDatasetDefinition,
		// "startDate=${startDateGC},endDate=${endDateGC}"));

		// HIVARTRETDatasetDefinition hivArtRetDatasetDefinitionNet = new
		// HIVARTRETDatasetDefinition();
		// hivArtRetDatasetDefinitionNet.addParameters(getParameters());
		// hivArtRetDatasetDefinitionNet.setNetRetention(true);
		// reportDefinition
		// .addDataSetDefinition(
		// "HMIS:Number of persons on ART in the original cohort including those
		// transferred in, minus those transferred out (net current cohort)",
		// map(hivArtRetDatasetDefinitionNet,
		// "startDate=${startDateGC},endDate=${endDateGC}"));

		// HIVLinkageNewCtDatasetDefinition linkageNewDataset = new
		// HIVLinkageNewCtDatasetDefinition();
		// linkageNewDataset.setParameters(getParameters());
		// reportDefinition.addDataSetDefinition(
		// "HMIS:Linkage outcome of newly identified Hiv positive individuals in the
		// reporting period",
		...
		// hivPvlsDataset
		// .setDescription("Number of adult and pediatric ART patients for whom viral
		// load test result received in the reporting period (with in the past 12
		// months)");
		// reportDefinition
		// .addDataSetDefinition(
		// "HMIS:Viral load Suppression (Percentage of ART clients with a suppressed
		// viral load among those with a viral load test at 12 month in the reporting
		// period)",
		...
		// hivPvlsUnDataset
		// .setDescription("Total number of adult and paediatric ART patients with an
		// undetectable viral load(<50 copies/ml) in the reporting period (with in the
		// past 12 months)");
		// reportDefinition
		// .addDataSetDefinition(
		// "HMIS:(UN) Viral load Suppression (Percentage of ART clients with a
		// suppressed viral load among those with a viral load test at 12 month in the
		// reporting period)",
		...
		// hivPvlsLvDataset
		// .setDescription("Total number of adult and paediatric ART patients with low
		// level viremia (50 -1000 copies/ml) in the reporting period (with in the past
		// 12 months)");
		// reportDefinition
		// .addDataSetDefinition(
		// "HMIS: (LV) Viral load Suppression (Percentage of ART clients with a
		// suppressed viral load among those with a viral load test at 12 month in the
		// reporting period)",
		...
		// reportDefinition.addDataSetDefinition(
		// "HMIS:Proportion of PLHIV currently on differentiated service Delivery model
		// (DSD) in the reporting period",
		...
		reportDefinition.addDataSetDefinition(
				"HMIS:Number of individuals receiving Pre-Exposure Prophylaxis",
