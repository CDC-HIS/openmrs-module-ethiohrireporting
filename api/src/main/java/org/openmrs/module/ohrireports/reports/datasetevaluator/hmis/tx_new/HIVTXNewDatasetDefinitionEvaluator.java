...
public class HIVTXNewDatasetDefinitionEvaluator implements DataSetEvaluator {
	...
	public void buildDataSet(SimpleDataSet dataSet) {
		...
		//>=50
		dataSet.addRow(buildColumn(".31", ">=50 year, Male",
		new QueryParameter(50D, 200D, "M", UNKNOWN)));
		
		dataSet.addRow(buildColumn(".33", ">=50 year, Female-non-pregnant",
	...
	private Integer getHTXNew(QueryParameter parameter) {
		...
		if (parameter.maxAge == 0 && parameter.minAge == 0) {
			cohort = patientQuery.getOnArtCohorts(parameter.gender, _datasetDefinition.getStartDate(),
					_datasetDefinition.getEndDate(), cohort);
			persons = patientQuery.getPersons(cohort);
			int count = cohort.size();
			int countM = cohort.getMemberIds().size();
			int person = persons.size();
